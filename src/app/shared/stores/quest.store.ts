...
import {
  QuestDomainService,
  QuestContext
} from '../../features/quest/services/quest-domain.service';
import {
  QuestStep,
  QuestStepType
} from '../../features/quest/models/quest.model';
  ...
  withMethods((store) => {
    ...
    return {
      ...
      processNextQuestStep(): void {
        ...
        const nextStep = questDomainService.generateNextStep(
          currentHero,
          context
        );
        if (!nextStep) {
        ...
        if (
          nextStep.type === QuestStepType.ENCOUNTER &&
          nextStep.combatResult
        ) {
          // Calculate health damage from combat
          const initialHeroHealth = currentHero.health;
          const combatFinalHealth =
            nextStep.combatResult.turns.length > 0
              ? nextStep.combatResult.turns[
                  nextStep.combatResult.turns.length - 1
                ].heroHealthAfter
              : initialHeroHealth;
      ...
      finalizeQuest(context: QuestContext): void {
        ...
        const questResult = questDomainService.createQuestResult(
          context,
          steps
        );
        ...
        if (questResult.questStatus === 'successful') {
          if (questResult.gooGained && questResult.gooGained > 0) {
            stationFacade.addGoo(
              questResult.gooGained,
              'Quest completion reward'
            );
          }

          if (questResult.metalGained && questResult.metalGained > 0) {
            stationFacade.addMetal(
              questResult.metalGained,
              'Quest completion reward'
            );
          }
