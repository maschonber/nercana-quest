...
import { Monster, MonsterType, CombatAbility } from '../models/monster.model';
...
describe('QuestDomainService', () => {
  ...
  beforeEach(() => {
    ...
    monsterServiceSpy.generateMultiMonsterEncounter.mockReturnValue([
      mockMonster
    ]);
  ...
  describe('Quest Generation System', () => {
    it('should create quest context with appropriate properties', () => {
      ...
      expect(context).toHaveProperty('questStatus');
      ...
      for (let i = 0; i < attempts; i++) {
        ...
        if (context.questStatus === 'successful') {
          ...
          totalExperience += steps.reduce(
            (sum, step) => sum + step.experienceGained,
            0
          );
        }
      }
      ...
      for (let i = 0; i < 10; i++) {
        ...
        while (context.remainingStepTypes.length > 0) {
          ...
          if (step && step.type in stepMessages) {
            stepMessages[step.type as keyof typeof stepMessages].add(
              step.message
            );
          }
        }
    ...
    it('should include combat data for encounter steps', () => {
      ...
      const encounterSteps = steps.filter(
        (step) => step.type === QuestStepType.ENCOUNTER
      );
      ...
      if (encounterSteps.length > 0) {
        encounterSteps.forEach((step) => {
          ...
          if (step.monster && step.combatResult) {
            expect(
              monsterServiceSpy.generateMultiMonsterEncounter
            ).toHaveBeenCalled();
            expect(combatServiceSpy.createTeamCombat).toHaveBeenCalled();

            // Verify combat rewards match step rewards
            expect(step.experienceGained).toBe(
              step.combatResult.experienceGained
            );
